...
interface ICustomCareStaffDialogProps extends RouteComponentProps {
  ButtonMenu: React.ReactElement;
  isEdit?: boolean;
  editField?: IUser | null;
}
// haven't design
const CustomCareStaffDialog: React.FC<ICustomCareStaffDialogProps> = props => {
  ...
  const [permissionSelected, setPermissionSelected] =
    useState<IPermissionItem | null>(PERMISSION.CARE_STAFF);
  ...
  const handleSubmit = () => {
    const userInput: IUserInput = {
      ...
      permission: permissionSelected?.type || "CARE_STAFF",
    ...
    switch (permissionSelected?.type) {
      case "ADMIN":
        ...
        break;
      case "MANAGER":
        history.push(PATH.USER_MANAGEMENT.SYSTEM_STAFF);
        break;
      case "USER":
        ...
        <DialogTitle className="mb-4">
          <p className="text-xl text-black normal-case laptop:text-mxl">
            {isEdit
              ...
              : "Thêm tài khoản nhân viên chăm sóc"}
          </p>
        </DialogTitle>
...
export default withRouter(CustomCareStaffDialog);
