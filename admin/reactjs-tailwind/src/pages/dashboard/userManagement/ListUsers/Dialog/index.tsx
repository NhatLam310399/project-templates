...
import { IPermissionItem, PERMISSION, LIST_GENDER } from "constants/users";
...
interface IAdminDialogProps extends RouteComponentProps {
  ButtonMenu: React.ReactElement;
  editField?: IUser | null;
  isEdit?: boolean;
}
const UserDialog: React.FC<IAdminDialogProps> = props => {
  const { ButtonMenu, editField, history, isEdit } = props;
  ...
  const [permissionSelected, setPermissionSelected] =
    useState<IPermissionItem | null>(PERMISSION.USER);
  ...
  const handleSubmit = () => {
    const userInput: IUserInput = {
      ...
      permission: permissionSelected?.type || "USER",
